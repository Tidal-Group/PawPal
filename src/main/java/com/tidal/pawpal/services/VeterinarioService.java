...
public class VeterinarioService extends VeterinarioServiceContract {
    ...
    public void elimina(Long id) {
        super.elimina(id, (veterinario) -> {
            ...
            // una copia dell'array evita una ConcurrentModificationException
            List<Specie> specieToCleanup = new ArrayList<>(veterinario.getSpecieTrattate());
            for(Specie specie : specieToCleanup) veterinario.removeSpecie(specie);
            // una copia dell'array evita una ConcurrentModificationException
            List<Prestazione> prestazioniToCleanup = new ArrayList<>(veterinario.getPrestazioniOfferte());
            for(Prestazione prestazione : prestazioniToCleanup) veterinario.removePrestazione(prestazione);
        });
    }
